...
public:
    ...
    fileCommandFacility(std::string uri) : CommandFacility(uri) {
        ...
        auto puri = dunedaq::url::parse(uri);

        if (!(puri.scheme.empty() || puri.scheme == "file")) {
        ...
        auto dot = puri.path.find_last_of(".");
        std::string fmt = puri.path.substr(dot+1);
        auto fmtitr = puri.queries.find("fmt");
        if (fmtitr != puri.queries.end()) {
            fmt = fmtitr->second;
        }
        if (fmt.empty()) {
        ...
        istr.open(puri.path, std::ios_base::in);
        if (fmt == "json") {
            ios.reset(new JsonArray(puri.path, istr, true));
            return;
        }
        if (fmt == "jstream") {
            ios.reset(new JsonStream(puri.path, istr));
            return;
        }
