...
ERS_DECLARE_ISSUE(cmdlib, UnsupportedUri, "Unsupported URI: " << uri, ((std::string)uri))
...
ERS_DECLARE_ISSUE(cmdlib, CannotParseCommand, "Failed to parse command: " << error, ((std::string)error))

ERS_DECLARE_ISSUE(cmdlib, CommandExecutionFailed, "Execution of command failed: " << error, ((std::string)error))
